...
Shader "Hidden/HBAO"
{
	...
	CGINCLUDE
		...
		inline float4 FetchSceneColor(float2 uv) {
			#if UNITY_SINGLE_PASS_STEREO
			float4 col = tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(uv));
			#else
	...
	SubShader {
		...
		Pass {
			CGPROGRAM
				...
				float4 frag (v2f i) : SV_Target {
					...
					float4 col = FetchSceneColor(i.uv);
		...
		// 32: combine deferred HDR (multiplicative blending)
		Pass {
		...
		// 34: combine integrated HDR (multiplicative blending)
		Pass {
			Blend DstColor Zero
			CGPROGRAM
		...
		// 35: combine color bleeding HDR (additive blending)
		Pass {
			Blend One One
			CGPROGRAM
				...
				half4 frag_blend(v2f i) : SV_Target {
					return half4(FetchOcclusion(i.uv2).rgb, 1.0);
				}

			ENDCG
		}

		// 36: AO debug pass (additive blending)
		Pass {
			Blend One Zero
			CGPROGRAM
		...
		// 37: AO debug pass (multiplicative blending)
		Pass {
			Blend DstColor Zero
			CGPROGRAM
